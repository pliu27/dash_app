...
app.layout = html.Div([
	                  ...
	                  dbc.Row([dbc.Col(html.Div([html.H6(id = 'pie_clicked', children='pie selected'),
	                  	                        dash_table.DataTable(id = 'table_pie',
	                  	                        	                 columns = [{'name': i, 'id': i, 'deletable': True} for i in ['city','customer']],
...
@app.callback(
   [Output(component_id='pie_clicked', component_property='children'),
    Output(component_id='table_pie', component_property='data')],
   [Input(component_id='graph2', component_property='clickData'),
    Input(component_id='table1', component_property='data'),
    Input(component_id='machine', component_property='value')]
)
def update_table_pie(click_data, sub_table, machine):
	if click_data != None:
		selected = click_data['points'][0]['label']
		dff = pd.DataFrame.from_dict(sub_table)
		pie_df = dff[dff['city'] == selected].reset_index(drop=True)[['city','customer']]
		return selected, pie_df.to_dict(orient = 'records')
	else:
		return '', [] # pd.DataFrame(columns = ['city','customer']).to_dict('records')
